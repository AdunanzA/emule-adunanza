...
void CUpDownClient::DrawStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	s_StatusBar.Fill(crNeither);
		...
		&& (m_abyPartStatus 
			|| m_abyIncPartStatus	//>>> ::ICS [enkeyDEV]
			|| m_abySeenPartStatus	//>>> ::AntiHideOS [netfinity]
			)
		) 
	{
		COLORREF crBoth;
		...
		COLORREF crClientPartial = RGB(170, 50, 224); //>>> ::ICS [enkeyDEV]
		COLORREF crClientSeen = RGB(150, 240, 240); //>>> ::AntiHideOS [netfinity]
		if (g_bLowColorDesktop) {
			crBoth = RGB(0, 0, 0);
...
//>>> ::Optimization
	if ((GetIP()!=0 && GetIP() == tocomp->GetIP())
		...
		|| (GetConnectIP()!=0 && GetConnectIP() == tocomp->GetConnectIP()))
	{
		//The IP of both match
		if (GetUserPort()!=0 && GetUserPort() == tocomp->GetUserPort())
...
void CUpDownClient::ProcessFileStatus(bool bUdpPacket, CSafeMemFile* data, CPartFile* file)
{
	...
	else
	{
		...
		uint16 owned = 0; //>>> : Anti HideOS
		const bool checkSeenParts = IsPartialSource(); //>>> ::AntiHideOS [netfinity]
		...
		m_bCompleteSource = (owned == m_nPartCount); //>>> : Anti HideOS
	}
...
void CUpDownClient::ProcessHashSet(const uchar* packet, uint32 size, bool bFileIdentifiers)
{
	...
	if (bFileIdentifiers)
	{
		...
		bool bPerformFirstHash = true;		// : SafeHash - Rehash
		if (m_fHashsetRequestingAICH && !bAICH)
		{
			...
			reqfile->SetAICHHashSetNeeded(true);
			bPerformFirstHash = false;		// : SafeHash - Rehash
		}
		...
		m_fHashsetRequestingMD4 = 0;
		m_fHashsetRequestingAICH = 0;
		if(bPerformFirstHash) reqfile->PerformFirstHash();		// : SafeHash - Rehash
	}
	else
	{
		if (!m_fHashsetRequestingMD4)
			throw CString(_T("unwanted hashset"));
		if ( (!reqfile) || !md4equ(packet,reqfile->GetFileHash()))
		{
			CheckFailedFileIdReqs(packet);
		...
		reqfile->PerformFirstHash();		// : SafeHash - Rehash
	}
	SendStartupLoadReq();
}
...
//dazzle : [CreateBlockRequests] : CUpDownClient : if no blocks available, try to drop the slowest source, then try again
		...
		if(AduStream) { //  -> Fonti lente non vanno bene nello streaming
...
void CUpDownClient::ProcessFileIncStatus(CSafeMemFile* data, const UINT size, const bool readHash)
{
	...
	if (readHash)
	{
		...
		if(!md4equ(cfilehash, reqfile->GetFileHash()))
