	...
	while (pos != NULL){
		...
		switch( pCurrent->m_eItemType ){
			...
			case SDI_TEMP:
				...
				break;
			case SDI_DIRECTORY: {
				//  START: shareSubdir - do both lists as well
				/*
				// add subdirectories
				bool bShowWarning = false;
...
{	
	...
	if (pSelectedDir != NULL && pSelectedDir->m_eItemType != SDI_UNSHAREDDIRECTORY && pSelectedDir->m_eItemType != SDI_FILESYSTEMPARENT){
		...
		m_ShareDirsMenu.EnableMenuItem(MP_SHAREDIR, !FileSystemTreeIsShared(pSelectedDir->m_strFullPath, true, true) ? MF_ENABLED : MF_GRAYED);	// : shareSubdir - don't suggest double sharing
...
//								m_pSharedFilesCtrl->UpdateFile(file); //>>> ::Optimization
								break;
							case MP_PRIOLOW:
//								file->SetAutoUpPriority(false); //>>> ::Optimization
...
//								m_pSharedFilesCtrl->UpdateFile(file);  //>>> ::Optimization
						...
						m_pSharedFilesCtrl->UpdateFile(file, false, &bSelected); //>>> ::Optimization
					}
//>>> ::Optimization
					if(bSelected)
						theApp.emuledlg->sharedfileswnd->ShowSelectedFilesDetails();
...
void CSharedDirsTreeCtrl::RemoveSharedDirectory(CString strDir, bool bSubDirectories){
	...
	//  START: shareSubdir - check second list
...
void CSharedDirsTreeCtrl::FileSystemTreeSetShareState(const CDirectoryItem* pDir, bool bSubDirectories){
	if (m_bUseIcons && pDir->m_htItem != NULL)
		//MORPH START - Changed, ShareSubDir []
		/*
...
void CSharedDirsTreeCtrl::EditSharedDirectories(const CDirectoryItem* pDir, bool bAdd, bool bSubDirectories){
	...
	//  START: shareSubdir - sync second list
