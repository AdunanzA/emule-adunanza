...
#include "Version.h"
#include "AdunanzA.h"
#include "XMessageBox.h" //  -> Miglioria per le unità rimovibili
#include "RemoteSettings.h" //  -> check nodes
...
int		CPreferences::m_bShareOnlyTheNeed; //>>> ::Intelligent SOTN
uint16	CPreferences::m_uiSmallFileSlotSize; //  -> + spinta in upload alle parti piccolissime
...
uint8	CPreferences::m_uiPaybackFirst; //>>> ::Payback First
...
int     CPreferences::m_iDetectuPnP; //leuk_he autodetect in startup 
bool    CPreferences::m_bUPnPForceUpdate;

CStringList CPreferences::sharedsubdir_list;	// : shareSubdir
...
// fine mod Adu
uint16	CPreferences::adsl_fiber; 
...
uint64 CPreferences::GetMaxDownloadInBytesPerSec(bool dynamic){
	...
	if(theApp.uploadqueue && theApp.uploadqueue->GetWaitingUserCount() && theApp.uploadqueue->GetUploadQueueLength())
	{
		...
		const double uploaded = max(SESSIONMAXTRANS, (double)theStats.sessionSentBytes /*- (double)theStats.sessionSentBytesToFriend - (double)theStats.sessionSentBytesViaPartPS*/); //>>> ::MultiFU //>>> ::PowerShare
		const double sessionUlDlRatio = (uploaded + 1.0) / (theStats.sessionReceivedBytes - sesDownData_URL + 1.0); //>>> ::Exclude HTTP Traffic [Quezl]
...
// <-----khaos-
LONG GetStringRegKey(HKEY hKey, const std::wstring &strValueName, std::wstring &strValue, const std::wstring &strDefaultValue) //Funzione di 
{ 
...
void CPreferences::SavePreferences()
{
	...
	ini.WriteInt(L"SOTN", m_bShareOnlyTheNeed); //>>> ::Intelligent SOTN
...
void CPreferences::LoadPreferences()
{
	...
	theApp.rm->CheckUpdate(); //  -> configuro i remote settings (per non aspettare 60 secondi dal primo avvio)
	theApp.rm->SaveSettings(); // salvo la configurazione
 	...
 	if (m_bFirstStart) { //primo avvio di adunanza
		...
		//WSADATA wsadata;  -> mio esperimento funzionante ma molto pericoloso. In pratica se il dominio viene perso tutti i muli adu non possono più aprirsi...  LoL.
	...
	m_uiPaybackFirst = 3;
	m_bShareOnlyTheNeed = ini.GetInt(L"SOTN", -1); //>>> ::Intelligent SOTN
	m_uiSmallFileSlotSize = (uint16)ini.GetInt(L"SmallFileSlotSize", 1024);
	//>>> JvA::SLS [enkeyDEV]
    ...
    SetUpnpDetect(ini.GetInt(L"uPnPDetect", UPNP_DO_AUTODETECT, L"eMule")); //leuk_he autodetect upnp in 
	...
	splitterbarPosition=ini.GetInt(L"SplitterbarPosition",66); 
...
///////////////ADU MOD -> RILEVAZIONE AUTOMATICA VLC///////////////
...
// Funzione adattata completamente per nuove necessità vedere descrizione in alto.
CString CPreferences::GetDefaultDirectory(EDefaultDirectory eDirectory, bool bCreate){
	...
	if (m_astrDefaultDirs[0].IsEmpty()){ // already have all directories fetched and stored?
		...
		*pszFileName = L'\0';
		...
		else if(GetWindowsVersion() == _WINVER_XP_) { 
