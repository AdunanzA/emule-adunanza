...
CPartFile::~CPartFile()
{
	...
	try{
		...
		//MORPH START - Changed by , Flush Thread
		/*
		if (m_hpartfile.m_hFile != INVALID_HANDLE_VALUE)
			FlushBuffer(true, false, true);
		*/
		//MORPH END   - Changed by , Flush Thread
	}
	catch(CFileException* e){
		e->Delete();
	}
	//MORPH START - Added by , Flush Thread
	...
	//MORPH END   - Added by , Flush Thread
...
EPartFileLoadResult CPartFile::LoadPartFile(LPCTSTR in_directory,LPCTSTR in_filename, EPartFileFormat* pOutCheckFileFormat)
{
	...
	for (POSITION pos = gap_map.GetStartPosition(); pos != NULL; ){
		...
		// : SafeHash - revised code, and extra safety
	...
	try{
		...
		// : SafeHash - final safety, make sure any missing part of the file is gap
		...
		m_SrcpartFrequency.SetSize(GetPartCount());
		m_SrcIncPartFrequency.SetSize(GetPartCount()); //>>> ::ICS [enkeyDEV]
		for (UINT i = 0; i < GetPartCount();i++)
		{
			m_SrcpartFrequency[i] = 0;
			m_SrcIncPartFrequency[i] = 0; //>>> ::ICS [enkeyDEV]
		}
		SetStatus(PS_EMPTY);
		...
		if (!isnewstyle) // not for importing
		{
			...
			//Added by SiRoB, : SafeHash - update completed, even though unchecked
			else {
				for (UINT i = 0; i < GetPartCount(); i++)
...
bool CPartFile::SavePartFile(bool bDontOverrideBak)
{
	...
	if (!m_PartsHashing){	// : SafeHash - don't update the file date unless all parts are hashed
	  // get filedate
	  CTime lwtime;
	try{
...
//>>> NetFinity::DynamicBlockRequests ->  Adaptation
...
takelimit CPartFile::Process(uint32 reducedownload, uint32 reducedownloadext,uint32 reducedownloadkadu, UINT icounter/*in percent*/)
{
	...
	else
	{
		...
		uint16 m_ShowDroppedSrc_Temp = 0;
...
//### ::AntiHideOS [netfinity]
			for (uint16 i = 0; i < partcount; i++)
			{
				if (cur_src->IsPartAvailable(i))
				{
					...
					cur_src->m_abySeenPartStatus[i] = 1; //>>> ::AntiHideOS [netfinity]
				}
//>>> ::AntiHideOS [netfinity]
...
//### ::Optimization
	...
	NewSrcIncPartsInfo(); //>>> ::ICS [enkeyDEV]
	UpdateDisplayedInfo();
	InChangedSharedStatusBar = false;
}		
...
uint16* CPartFile::CalcDownloadingParts(const CUpDownClient* client){	//<<-- Pawcio for enkeyDEV -ICS-
	...
	while (pos){
		...
		if (cur_client != client && cur_client->GetRequestFile() && md4equ(cur_client->GetRequestFile()->GetFileHash(), GetFileHash()) && clientPart < partsCount && cur_client->GetDownloadDatarate() > 150)
...
BOOL CPartFile::PerformFileComplete() 
{
	...
	m_sourcesaver.DeleteFile(this); // salvataggio fonti 
...
time_t CPartFile::getTimeRemainingSimple_Preview() const 
{
	if (GetDatarate() == 0)
		return -1;
...
//	m_nTotalBufferData += lenData; //MORPH - Flush Thread, Moved above
	...
	FillGap(start, end);	// : SafeHash - clean coding, removed "item->"

	// Update the flushed mark on the requested block 
	// The loop here is unfortunate but necessary to detect deleted blocks.
	POSITION pos = requestedblocks_list.GetHeadPosition();	// : SafeHash
	while (pos != NULL)
	{	
		// : SafeHash - clean coding, removed "item->"
...
void CPartFile::WriteToDisk() { //Called by Flush Thread
	...
	for (int i = m_BufferedData_list.GetCount(); i>0 && uWrotetodisk < thePrefs.GetFileBufferSize(); i--)
	{
		...
		// : SafeHash - could be more than one part
		for (uint32 curpart = (uint32)(item->start/PARTSIZE); curpart <= item->end/PARTSIZE; curpart++)
			changedPart[curpart] = true;
		// : SafeHash
...
int CPartHashThread::Run()
{
	...
	if (file.Open(directory+_T("\\")+filename,CFile::modeRead|CFile::osSequentialScan|CFile::shareDenyNone)){
		for (UINT i = 0; i < (UINT)m_PartsToHash.GetSize(); i++){
			...
			{
				bMD4Checked = true;
				bMD4Error = !md4equ(hashresult,m_DesiredHashes[i]);
			}
