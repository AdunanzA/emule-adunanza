...
int CAddFileThread::Run()
{
	...
	if (newrecord->CreateFromFile(m_strDirectory, m_strFilename, m_partfile) && theApp.emuledlg && theApp.emuledlg->IsRunning()) // : SafeHash - in case of shutdown while still hashing
	{
	...
	else
	{
		...
		// : SafeHash - inform main program of hash failure
...
/* old code:
	...
	for (int ix=1;ix<thePrefs.GetCatCount();ix++)
	{
		...
		if( l_sAdded.Find( ltempDir ) == NULL && //ssd
			!FindStartingWith(&l_sAddedWithSubdir, ltempDir) ) {	// : shareSubdir
...
void CSharedFileList::AddFilesFromDirectory(const CString& rstrDirectory, bool bWithSubdir)	// : shareSubdir
{
	CFileFind ff;
...
{
	bool bAdded = false;
	RemoveFromHashing(toadd);	// : SafeHash - hashed ok, remove from list, in case it was on the list
...
{
	m_dwLastReloaded = ::GetTickCount(); //>>> ::Simple Auto-Reload
	// : SafeHash - don't allow to be called until after the control is loaded
	if (!output)
		return;
	// : SafeHash
	...
	/* : SafeHash remove - check moved up
	if (output)
	*/
...
void CSharedFileList::SetOutputCtrl(CSharedFilesCtrl* in_ctrl)
{
	...
	output->ReloadFileList();
	/* official 
	HashNextFile();		// : SafeHash - if hashing not yet started, start it now
	*/
	LoadSingleSharedFilesList(); // MORPH : SafeHash - load shared files after everything
	Reload();		// MORPH : SafeHash - load shared files after everything
}
...
void CSharedFileList::HashNextFile(){
	...
	UnknownFile_Struct* nextfile = waitingforhash_list.RemoveHead();
	currentlyhashing_list.AddTail(nextfile);	// : SafeHash - keep track
	...
	// : SafeHash - nextfile deleting handled elsewhere
	//delete nextfile;
}
...
{
	ReloadIfNeeded(); //>>> ::Simple Auto-Reload
	Publish();
...
bool CSharedFileList::ExcludeFile(CString strFilePath)
{
	...
	bool bSingleShared = bShared; //>>> taz::Automatic shared files updater [MoNKi/Stulle/]
	...
	// add to exclude list
	if (!bSingleShared) //>>> taz::Automatic shared files updater [MoNKi/Stulle/]
...
// : shareSubdir
		//  START: shareSubdir - recursive call
	...
	else
	{
		...
		// : SafeHash - don't double hash, MY way
