...
extern LONG GetStringRegKey(HKEY hKey, const std::wstring &strValueName, std::wstring &strValue, const std::wstring &strDefaultValue);
extern inline void AfxEnableDlgItem(CWnd *pDlg, int nIDDlgItem, BOOL bEnabled = TRUE);
...
bool WizardAdunanzA() {
	...
	if (isNT()) // Configurazione automatica supportata solo dai moderni OS. Almeno per ora... Causa proposta VLC che è incompatibile con sistemi < NT
		sheet.AddPage(&page2);
	...
	thePrefs.m_bUPnPNat = true; //uPnP attivato di default.
	thePrefs.m_bShowDwlPercentage = true; // metto la percentuale dei download di default 
	thePrefs.m_bCryptLayerSupported = true; // Supportato per default - sarebbe l'offuscamento su ed2k.
	thePrefs.notifierOnDownloadFinished = true; // notifica download completi.
	thePrefs.notifierOnImportantError = true; // Mi serve per lo streaming.
...
BOOL AduWizard::PreTranslateMessage(MSG* pMsg) { // blocco i tasti f4 e escape per evitare che un furbetto chiuda la finestra modale inaspettatamente.
...
{
	extern bool updating;
	updating = true; //serve per fare in modo che il downloaddialog sia centrato :3
	try {
		GetDlgItem(IDC_BUTTON2)->EnableWindow(false);
		GetDlgItem(IDC_BUTTON1)->EnableWindow(false);
...
{
    SetTimer(0x1000, 1000, 0); // Ogni secondo faccio il controllo dell'installer vlc per sapere se è in esecuzione o meno
}
...
void CPPgWiz1General::OnTimer(UINT_PTR nIDEvent )
{
