...
CBarShader::CBarShader(uint32 height, uint32 width) {
	...
	m_Spans.SetAt(0, 0);	//speedBarShader
	m_Modifiers = NULL;
	m_bIsPreview=false;
}
...
void CBarShader::Draw(CDC* dc, int iLeft, int iTop, bool bFlat) {
	POSITION pos = m_Spans.GetHeadPosition();	// : speedBarShader
	RECT rectSpan;
	rectSpan.top = iTop;
	...
	m_Spans.GetNext(pos);
	//  speedBarShader
	while(pos != NULL && rectSpan.right < (iLeft + m_iWidth)) {	// speedBarShader
		uint64 uSpan = m_Spans.GetKeyAt(pos) - start;	// speedBarShader
		uint64 uPixels = (uint64)(uSpan * m_dPixelsPerByte + 0.5f);
		if (uPixels > 0) {
			rectSpan.left = rectSpan.right;
			rectSpan.right += (int)uPixels;
			FillRect(dc, &rectSpan, color, bFlat);	// speedBarShader
