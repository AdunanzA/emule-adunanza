...
void CSharedFilesCtrl::Init()
{
	...
	InsertColumn(18,GetResString(IDS_SOTN),				LVCFMT_LEFT,  100);  //>>> ::Intelligent SOTN
	...
	if (tooltip){
		...
		SubclassWindow(m_pToolTip->GetSafeHwnd(), tooltip);
...
BOOL CSharedFilesCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	CTypedPtrList<CPtrList, CKnownFile*> selectedKnown; //>>> ::PowerShare
	POSITION pos = GetFirstSelectedItemPosition();
	while (pos != NULL){
			...
			case MP_PRIOAUTO:
				{
					...
					while (pos != NULL)
					{
						...
						file->SetAutoUpPriority(wParam == MP_PRIOAUTO); //>>> ::Optimization
						switch (wParam) 
						{
							case MP_PRIOVERYLOW:
//								file->SetAutoUpPriority(false); //>>> ::Optimization
								file->SetUpPriority(PR_VERYLOW);
//								UpdateFile(file); //>>> ::Optimization
								break;
							case MP_PRIOLOW:
//								file->SetAutoUpPriority(false); //>>> ::Optimization
...
//								UpdateFile(file); //>>> ::Optimization
								break;	
							case MP_PRIOAUTO:
//								file->SetAutoUpPriority(true); //>>> ::Optimization
...
void CSharedFilesCtrl::CreateMenues()
{
	...
	if (m_PowerSharedMenu) VERIFY( m_PowerSharedMenu.DestroyMenu() ); //>>> ::PowerShare
...
//>>> ::Intelligent SOTN
	...
	m_PowerSharedMenu.AppendMenu(MF_SEPARATOR);
	m_PowerSharedMenu.AppendMenu(MF_STRING, GUIDA_FEATURES, _T("Informazioni!"), L"Info"); 
//### ::Intelligent SOTN
//### ::PowerShare
	...
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_POPUP,(UINT_PTR)m_PowerSharedMenu.m_hMenu, _T("AdunanzA"), L"PULSANTEA3"); //>>> ::PowerShare
	m_SharedFilesMenu.AppendMenu(MF_STRING|MF_SEPARATOR);
