...
#include "MuleStatusBarCtrl.h"  percentuale hash
...
{	
	m_bShareOnlyTheNeed = true; //>>> ::Intelligent SOTN
	guifileupdatetime = ::GetTickCount() + 1000; //>>> ::PowerShare - CPU!
	m_bPowerShared = false; //>>> ::PowerShare
	m_iPartCount = 0;
	m_iED2KPartCount = 0;
...
void CKnownFile::UpdatePartsInfo()
{
	...
	for (UINT i = 0; i < partcount; i++) {
		...
		m_SOTNAvailPartFrequency[i] = 0; //>>> ::Intelligent SOTN
	}
	...
	for (POSITION pos = m_ClientUploadList.GetHeadPosition(); pos != 0; )
	{
		...
		cur_src->GetUploadingAndUploadedPart(m_AvailPartFrequency, m_SOTNAvailPartFrequency); //>>> ::Intelligent SOTN
	}
...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	m_SOTNAvailPartFrequency.SetSize(GetPartCount()); //>>> ::Intelligent SOTN
	for (UINT i = 0; i < GetPartCount();i++)
	{
		m_AvailPartFrequency[i] = 0;
		m_SOTNAvailPartFrequency[i] = 0; //>>> ::Intelligent SOTN
...
bool CKnownFile::LoadFromFile(CFileDataIO* file){
	// : SafeHash - load first, verify later
...
//this works pretty good but there is ONE big issue:
...
uint8* CKnownFile::GetPartStatus(CUpDownClient* client) const
{
	...
	bool bSOTN = GetShareOnlyTheNeed() && (!client || client->GetIncompletePartVersion() == 0); //>>> ::ICS [enkeyDEV]
//>>> Create tmp array
