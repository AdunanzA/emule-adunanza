...
void CClientCreditsList::SaveList()
{
	...
	while (pos)
	{
		...
		{
			if (!AduIsFastWebIP(cur_credit->m_dwIdentIP)) {
...
//  metto nel pattune la vecchia funzione di emule rendendola compatibile con i muli ufficiali.

const float CClientCredits::GetScoreRatio(const CUpDownClient* client) const //  -> e ne metto una più golosa xD
{
