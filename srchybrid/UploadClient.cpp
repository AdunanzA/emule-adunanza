...
float CUpDownClient::GetCombinedFilePrioAndCredit() 
{
    ...
    return 10.0f*credits->GetScoreRatio(this)*float(GetFilePrioAsNumber()); //  Crediti AdunanzA
}
...
void CUpDownClient::ResetRemainingUp()
{
	...
	if ((theApp.uploadqueue->waitinglist.IsEmpty() && IsAduClient()) || GetFriendSlot())
		m_nRemainingUp = (UINT)max(MIN_BLOCK_SIZE, min((uint64)_UI32_MAX, srcfile->GetFileSize()));
	else if(IsAduClient())
		m_nRemainingUp = (UINT)(max(MIN_BLOCK_SIZE, min(PARTSIZE, srcfile->GetFileSize())) * 2); 
	else
		m_nRemainingUp = (UINT)max(MIN_BLOCK_SIZE, min(PARTSIZE, srcfile->GetFileSize())); 
}
...
bool CUpDownClient::AddReqBlock(Requested_Block_Struct* reqblock, const bool bChecksNecessary)  //>>> ::Optimization
{
//>>> ::Optimization
	if(bChecksNecessary)
	{
...
//>>> ::Optimization
		...
		if(pDownloadingFile && pDownloadingFile->GetFileSize() < (uint64)MAXPRIORITYCOLL_SIZE) {
				if(!md4equ(reqblock->FileID, requpfileid))
				{
    ...
    for (POSITION pos = m_DoneBlocks_list.GetHeadPosition(); pos != 0; )
	{
		...
		if(md4equ(reqblock->FileID, cur_reqblock->FileID)) {
			if (reqblock->StartOffset >= cur_reqblock->StartOffset && reqblock->EndOffset <= cur_reqblock->EndOffset){
			    delete reqblock;
   				return true; //>>> ::Optimization - already added... no error
			}
        }
    }
	...
	return true; //>>> ::Optimization - alrighty :)
}
        ...
        while (!m_BlockRequests_queue.IsEmpty() && m_abyfiledata != (byte*)-2) 
		{
			...
			if (filedata_ReadFromDisk == NULL || m_BlockRequests_queue.GetCount()>1) {
				...
				else
				{
					...
					if (srcFile->IsPartFile() && !((CPartFile*)srcFile)->IsRangeShareable(currentBlock->StartOffset,currentBlock->EndOffset-1))	// : SafeHash - final safety precaution
					//MORPH END  - Changed by SiRoB, : SafeHash
					{
						CString error;
				...
				if (togo > m_nRemainingUp)	//  - Transfer Session Clipping
					togo = m_nRemainingUp;
