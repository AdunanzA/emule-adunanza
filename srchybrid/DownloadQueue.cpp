...
for (int i=0;i<thePrefs.tempdir.GetCount();i++) {
    	CStringList metsfound;	// : SafeHash - ensure each met is loaded once per tempdir
		...
		// : SafeHash
			...
			if (eResult == PLR_LOADSUCCESS)
			{
				...
				// : SafeHash remove - part files are shared later
				/*
...
//>>> ::Indicate already downloaded files
...
bool CDownloadQueue::AddDownload(CPartFile* newfile, bool paused) {
	...
	SetAutoCat(newfile);// HoaX_69 / : AutoCat
