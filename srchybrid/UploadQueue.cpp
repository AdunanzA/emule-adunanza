...
{	
	m_uiPaybackCount = 0; //>>> ::Payback First
	...
	m_dwFirewalledTimer = ::GetTickCount(); //>>> ::Automatic Firewalled Retries
	datarate = 0;
	counter=0;
...
CUpDownClient* CUploadQueue::FindBestClientInQueue(DWORD typeClient)
{
	...
	for (pos1 = waitinglist.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
		...
		if (cur_client->IsPowerShared()) //  -> i leechers si fottano. Non hanno il powershare.
		{
...
void CUploadQueue::InsertInUploadingList(CUpDownClient* newclient) 
{
	...
	//   -> per piacere ricordarsi la tabulazione del codice....
 ...
 *
 * This method is called approximately once every 100 milliseconds. () -> Porca Madonna
 */
...
bool CUploadQueue::RemoveFromUploadQueue(CUpDownClient* client, LPCTSTR pszReason, bool updatewindow, bool earlyabort)
{
	...
	//### ::Payback First
			...
			//client->SetCollectionUploadSlot(false);  -> non serve +
...
bool CUploadQueue::CheckForTimeOver(CUpDownClient* client)
{
	if(client->GetFriendSlot())
	{
		...
		if(NULL == remain) //  -> Se la quantità di byte arriva a 0, stacca il client.
			return true;
		else
			return false;
	}
	...
	if(0 == remain) //  -> Se la quantità di byte arriva a 0, stacca il client.
		return true;
...
VOID CALLBACK CUploadQueue::UploadTimer(HWND /*hwnd*/, UINT, UINT_PTR, DWORD /*dwTime*/)
{
