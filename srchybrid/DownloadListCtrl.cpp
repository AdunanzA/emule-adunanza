...
void CDownloadListCtrl::ControllaStreaming() { //  -> Implementazione Streaming
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			m_FileMenu.EnableMenuItem(MP_STREAMING, (iSelectedItems == 1 && AduStream == 1 && file->IsMovie() && !file->GetCompletionError() && file->GetPercentCompleted() < 100) ? MF_ENABLED : MF_GRAYED);
			m_FileMenu.EnableMenuItem(MP_STARTSTREAM, (iSelectedItems == 1 && StreamOK && file->IsMovie() && !file->IsStopped() && file->IsReadyForPreview() && file->GetAduSourceCount() > 0 && file->GetPercentCompleted() > thePrefs.m_BufferStreaming) ? MF_ENABLED : MF_GRAYED);
			m_FileMenu.EnableMenuItem(MP_STOPSTREAM, (iSelectedItems == 1 && StreamSTOP && file->IsMovie() && !file->IsStopped()) ? MF_ENABLED : MF_GRAYED);
			m_FileMenu.EnableMenuItem(MP_INFO_STREAM, (iSelectedItems == 1) ? MF_ENABLED : MF_GRAYED);
			if(!AduStream) {
				if(file->GetPercentCompleted() == 100)
				m_FileMenu.EnableMenuItem(MP_STOPSTREAM, MF_ENABLED);
			}
		...
		m_FileMenu.EnableMenuItem(MP_STREAMING, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_INFO_STREAM, MF_GRAYED);
...
void CDownloadListCtrl::StoppaRiprendi(bool s_r) {
...
void CDownloadListCtrl::StopStreaming(CPartFile* file)
{
		// Invoking the Dialog
...
void CDownloadListCtrl::CreateMenues()
{
	...
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	m_FileMenu.AppendMenu(MF_STRING, MP_STREAMING, _T("AdunanzA Streaming Request"), _T("VLCICON"));
	m_FileMenu.AppendMenu(MF_STRING, MP_STARTSTREAM, _T("Start Streaming"), _T("DOWNLOAD"));
	m_FileMenu.AppendMenu(MF_STRING, MP_STOPSTREAM, _T("Stop Streaming"), _T("UPLOAD"));
	m_FileMenu.AppendMenu(MF_STRING, MP_INFO_STREAM, _T("Guida Streaming"), _T("INFO"));
	m_FileMenu.AppendMenu(MF_SEPARATOR);
