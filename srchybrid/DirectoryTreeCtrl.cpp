...
BEGIN_MESSAGE_MAP(CDirectoryTreeCtrl, CTreeCtrl)
	...
	ON_WM_CHAR()
	ON_WM_DESTROY()
	ON_MESSAGE(TM_FOUNDNETWORKDRIVE, OnFoundNetworkDrive)	// : shareSubdir - Multi-threading
END_MESSAGE_MAP()
...
void CDirectoryTreeCtrl::ShareSubDirTree(HTREEITEM hItem, BOOL bRecurse)
{
	...
	*/
	CheckChanged(hItem, !GetCheck(hItem), bRecurse==TRUE);	// : shareSubdir - Handled in a much cleaner fashion
...
void CDirectoryTreeCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_SPACE)
	{
		...
		if (hItem)
		{
			...
			if (!IsDisabled(hItem))	// : shareSubdir
...
void CDirectoryTreeCtrl::Init(void)
{
	...
	//  START: shareSubdir - local drives before network drives
	TVSORTCB tvs;
...
HTREEITEM CDirectoryTreeCtrl::AddChildItem(HTREEITEM hRoot, CString strText)
{
	...
	//  START: shareSubdir - always include lParam for sort
	if (hRoot == NULL) {
	...
	if (wWinVer != _WINVER_95_ && wWinVer != _WINVER_NT4_)
	{
		...
		else
		{
			...
			if (hRoot == NULL && shFinfo.szDisplayName[0] != _T('\0'))
			{
				...
				itInsert.item.lParam = (LPARAM)pti;
				*/
				//  START: shareSubdir remove - lParam already present
				strText = shFinfo.szDisplayName;
				strText.Trim();	// : shareSubdir - clean up whitespaces
				itInsert.item.pszText = strText.GetBuffer();
				itInsert.item.cchTextMax = strText.GetLength();
				//  END: shareSubdir remove - lParam already present
			}
		}
...
//  END : shareSubdir - allow checking indirect share
{
	if (strDir.Right(1) != _T('\\'))
		strDir += _T('\\');
	for (POSITION pos = m_lstShared.GetHeadPosition(); pos != NULL; )
...
void CDirectoryTreeCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	//  START: shareSubdir - add second list
...
//  START: shareSubdir - Multi-threading
