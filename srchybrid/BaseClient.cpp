...
{	
	m_abyUpPartStatusHidden = NULL; //>>> Intelligent SOTN
	//>>> ICS [enkeyDEV]
	m_abyIncPartStatus = NULL;
	m_incompletepartVer = 0;
	//### ICS [enkeyDEV]
	m_abySeenPartStatus = NULL; //>>> AntiHideOS [netfinity]
	m_abyfiledata = NULL;
	...
	// Mod Adu
 	...
 	aduver = 0;
 	// Fine adu
	hiscompletedparts_percent_up=-1; //percentuale upload.
...
void CUpDownClient::ClearHelloProperties()
{
	...
	m_fDirectUDPCallback = 0;
	m_fSupportsFileIdent = 0;
	m_incompletepartVer = 0; //>>> ICS [enkeyDEV]
}
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	const bool bSendICS = !m_pszUsername || GetIncompletePartVersion(); //>>> ::ICS [enkeyDEV]
...
{	
	...
	if (m_byDataCompVer == 0) {
		...
		m_byAcceptCommentVer = 0;
		m_nUDPPort = 0;
		m_incompletepartVer = 0;	//>>> ::ICS [enkeyDEV] //hmmm why is this reset?
	}
