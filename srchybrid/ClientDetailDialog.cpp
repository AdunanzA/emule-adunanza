...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		if (client->Credits()){
			...
			buffer.Format(_T("%.1f %+.1f [%.1f]"),(float)client->Credits()->GetScoreRatio(client)- (float)client->Credits()->GetBonusAdu(client),(float)client->Credits()->GetBonusAdu(client),(float)client->Credits()->GetMioScoreAdu(client->GetIP()));	//   crediti
